...
describe('quantilesPbci', () => {
  it('finds the quantiles for two arrays', async () => {
    ...
    const quantilesResults = await quantilesPbci({base, compare}, options);
  ...
  it('matches rogme quantiles_pbci output', async () => {
    const data = csvColumns(testFixtureTxt);
    const options = {nboot: 20_000, quiet: true, randomSeed: xorshiftSeed};
    const quantilesResults = await quantilesPbci(data, options);
