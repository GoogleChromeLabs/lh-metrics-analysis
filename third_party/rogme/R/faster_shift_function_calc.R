...
# Modified from https://github.com/GRousselet/rogme/blob/6582a8af4269be3773f111b57ce787892884b912/R/shift_function_calc.R

#' Compute the Harrell-Davis estimate of the qth quantile. Assumes weights are
#' precomputed, so not for use outside of this file.
#'
...
#' @param weights A vector of pbeta weights, of the same length as x.
#' @references
#' Harrell, F.E. & Davis, C.E. (1982) A new distribution-free quantile estimator. Biometrika, 69, 635-640.
hdFromWeights <- function(x, weights){
...
#'
#' @param x A vector of continuous observations. Missing values are not allowed.
#' @param y A vector of continuous observations. Missing values are not allowed.
#' @param x_name A name to use for the first group of observations - default = "Group1".
#' @param y_name A name to use for the second group of observations - default = "Group2".
#' @param nboot The number of bootstrap samples - default = 200.
#'
#' @return A data frame for the comparison, with one row per decile. The columns are: \itemize{
...
#' C2 <- C1 + rnorm(n)
#'
#' out <- shiftdhd(C1, C2) # use the default parameters
#' out <- shiftdhd(C1, C2, nboot = 500) # specify the number of bootstrap samples
...
shiftdhd <- function(x, y, x_name = "Group1", y_name = "Group2", nboot = 200) {
  ...
  boot_indices <- matrix(
    sample(len, size = len * nboot, replace = TRUE),
    nrow = nboot
  )
  xmat <- matrix(x[boot_indices], nrow = nboot, ncol = len)
  ymat <- matrix(y[boot_indices], nrow = nboot, ncol = len)

  # Harrell-Davis pbeta weights don't change for each decile (assuming same
  # length), so can pregenerate them. Indexed 1-9 for 10th through 90th deciles.
  hd_weights_by_d <- lapply(seq(1, 9), FUN = function(d) {
    q <- d / 10
    m1 <- (len + 1) * q
    m2 <- (len + 1) * (1 - q)
    vec <- seq(1, len)
    weights <- pbeta(vec / len, m1, m2) - pbeta((vec - 1) / len, m1, m2)
    return(weights)
  })

  # At this point, these samples are only used in sorted order (to multiply
  # against the hd weights). Presort so not re-sorting for every decile.
  x <- sort(x)
  y <- sort(y)
  xmat <- t(apply(xmat, 1, sort))
  ymat <- t(apply(ymat, 1, sort))

  # factor to correct for multiple comparisons
  crit <- 37 / len^(1.4) + 2.75
  # declare matrix for results
  m <- matrix(0, 9, 6)

  for (d in 1:9){ # decile loop
    q <- d / 10
    weights <- hd_weights_by_d[[d]]
    bvec <- apply(xmat, 1, hdFromWeights, weights) - apply(ymat, 1, hdFromWeights, weights)
    se <- sqrt(var(bvec))
    m[d,1] <- q
    m[d,2] <- hdFromWeights(x, weights)
    m[d,3] <- hdFromWeights(y, weights)
    m[d,4] <- m[d,2] - m[d,3] # difference
    m[d,5] <- m[d,4] - crit * se # difference - corrected SE
    m[d,6] <- m[d,4] + crit * se # difference + corrected SE
  }
  df <- data.frame(m)
  names(df) <- c("q", x_name, y_name, 'difference', 'ci_lower', 'ci_upper')
  return(df)
}
